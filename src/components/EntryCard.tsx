...
export default function EntryCard({
  ...
  return (
    <Link
      href={`/${locale}/entry/${entry.id}${returnTo ? `?returnTo=${encodeURIComponent(returnTo)}` : ""}`}
      ...
      <article className="content-auto">
        ...
        <div className="mt-3 flex items-start justify-between gap-3 border-t border-[var(--line)] pt-2">
          <div className="min-w-0">
            <p className="catalog-no truncate">
              {entry.volume ?? tradition.shortName}
            </p>
            <h3 className="mt-1 text-xl leading-[1.05] transition-colors">
              {primary}
            </h3>
          </div>
          <span
            className="catalog-no shrink-0 text-right"
            style={{ color: tradition.color }}
          >
            {typeLabels[entry.type][locale]}
            <br />
        ...
        <p className="mt-2 line-clamp-2 text-sm leading-relaxed text-vellum-dim">
          {entry.title}
        </p>
      </article>
    </Link>
