...
export function generateStaticParams() {
  return LOCALES.flatMap((locale) =>
    getTraditions().map((t) => ({ locale, id: t.id })),
  );
}
...
export async function generateMetadata({
  ...
  return {
    title: t.name[locale],
    description: t.intro?.[locale]?.slice(0, 140),
  };
}
...
export default async function TraditionPage({
  ...
  const traditionIndex =
    getTraditions().findIndex((tradition) => tradition.id === id) + 1;
    ...
    <div className="site-shell pt-10">
      ...
      <header className="grid gap-10 border-b border-[var(--line-strong)] pb-12 lg:grid-cols-[minmax(0,0.9fr)_minmax(24rem,1.1fr)]">
        <div className="flex flex-col justify-between">
          <div>
            <p className="eyebrow flex items-center gap-3">
              ...
              {String(traditionIndex).padStart(2, "0")}
            </p>
            <h1 className="mt-5 max-w-[11ch] text-6xl leading-[0.95] sm:text-8xl">
              {t.name[locale]}
            </h1>
          </div>
          <dl className="mt-10 grid grid-cols-2 gap-5 border-t border-[var(--line)] pt-4 text-sm text-vellum-dim">
            <div>
              <dt className="catalog-no">
                {locale === "zh" ? "神域所辖" : "Mythic realm"}
              </dt>
              <dd className="mt-1">{t.region[locale]}</dd>
            </div>
            <div>
              <dt className="catalog-no">
                {locale === "zh" ? "传诵纪元" : "Age remembered"}
              </dt>
              <dd className="mt-1">{t.period[locale]}</dd>
            </div>
            <div>
              <dt className="catalog-no">{dict.tradition.entriesIn[locale]}</dt>
              <dd className="mt-1 font-[family-name:var(--font-display-stack)] text-3xl text-vellum">
                {entries.length}
              </dd>
            </div>
          </dl>
        ...
        <div className="reading-rule">
          ...
          <Link href={atlasUrl(locale, t.id)} className="button-secondary mt-3">
      ...
      {groups.map((group) => (
        <section key={group.key} className="mt-16">
          <div className="section-heading">
            <h2
              className="text-3xl"
              style={group.label ? { color: t.color } : undefined}
            >
              {group.label ??
                (locale === "zh" ? "卷中诸名" : "Names within the leaves")}
            </h2>
