...
5. **image 字段只由配图管线写**，人和内容 agent 都别手填。Commons 图走 `scripts/fetch-image.mjs`，原创编辑插画走 `scripts/register-image.mjs`；雕版纹章 SVG（`Emblem.tsx`）只是制作期间的容错回退，不是任何正式条目的最终状态。

## 常用命令

```bash
npm run validate              # 全量校验数据（零 error 才能上）
npm run audit                 # 内容、断链、缺图、孤儿图、重复图与文风短语盘点
...
node scripts/register-image.mjs source.png --entry zeus --title "Original editorial illustration: Zeus" --artist "Myth Atlas · OpenAI image generation" --license "Original editorial artwork" --source-url "https://openai.com/"  # 本地原创图压缩+回填
```
...
- 优先使用 Wikimedia Commons 公有领域/CC 授权（PD/CC0/CC BY/CC BY-SA）；无可靠历史图版、存在文化语境风险或涉及现代版权角色时，可使用不冒充史料的原创编辑插画。两类图片都必须把来源、作者与授权性质写进 `entry.image`，并在条目页和 About 页展示。
...
3. **有版权的角色不做近似复刻**——指有明确在世作者、角色造型本身受版权保护的现代创作（警笛头 Trevor Henderson 2018、瘦长鬼影 Eric Knudsen 2009）。哪怕 Commons 上有粉丝画作并声称 CC 授权，上传者也无权授权角色本身的版权；此类条目改用传播媒介、相关场所或不包含角色造型的原创气氛图。
4. **会误导或冒犯时改变表现方式**——活态信仰采用相关地域、历史档案、物质文化或克制的象征性原创图；身份仍有学术争议的文物必须在图注中标明争议；只有“住客”图而没有本体图的山川地点采用地貌或古籍语境图。不得因为难配就把正式条目永久留空。
- 尺寸门槛：<380px 宽拒绝；体积门槛分级——宽度 ≥900px 只要 >5KB 就收，否则要 >20KB
- 压缩：sips 压到最长边 1000px JPEG q72（`scripts/compress-images.mjs` 可全量重压并回填 width/height）
- 所有正式条目都应有图；版权、文化语境或对象误配风险通过换用图像类型和准确图注解决
- 原创图的最终提示词、表现边界与登记方式归档在 `docs/IMAGE_PROVENANCE.md`
...
- **山名条目不拿住客冒充本体**：青丘之山、跂踵之山、崦嵫之山这类，古籍只画山里的异兽不画山。不能拿九尾狐去配「青丘之山」（而且会与 `jiu-wei-hu` 撞图）；应选与原文地貌相符的原创景观、古籍书页或明确标作语境图的材料。
...
- 主体是档案纸本：纸色 #e6dcc5 / 淡纸 #f2ead9 / 夜墨 #172022 / 朱砂 #9f3428 / 靛青 #243d54；夜墨只用于星图、创世时间线等沉浸区，每个传统另有自己的强调色（traditions.json.color）
- 字体：Cormorant（英文展示）/ EB Garamond（英文正文）/ Noto Serif SC（中文）/ IBM Plex Mono（编目号、坐标）
- 签名元素：巨幅志书刊头、纸本编目线、首页夜航星图（D3 naturalEarth 投影 + 谱系星标 + 传说坐标）、博物馆编目号 № NNN；神秘感来自抽象星轨、刻线与细密纹样，不套用某个具体文明的面具或图腾
